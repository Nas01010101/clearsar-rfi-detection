...
cross-fold ensemble that drops the domain-overlapping fold. On a
401-image leak-isolated meta-validation set the honest student source
built by this pipeline gains $+0.077$ mAP$_{50\text{-}95}$ over a cold
per-fold baseline (distillation $+0.040$, TTA $+0.036$, fold dropping
$+0.002$). The final submission reaches 0.4776 mAP$_{50\text{-}95}$ on
the held-out test set, placing 28th of 172 teams. We also report the
cautionary half of the story: on the genuinely unseen test set this
student source is largely redundant with the ensemble teacher, so the
full distillation--TTA--blend stack adds only $+0.0021$ over submitting
the teacher alone. All training used a single cloud NVIDIA A100 GPU.
\end{abstract}
...
images) and are never used for model selection. Honest generalisation
is judged by two separate instruments---a 401-image leak-isolated
meta-validation set sampled from the fold-0 validation pool (seed 42),
used only as a single-model gate, and the official held-out test set,
used for all model selection and final reporting.
...
\begin{table}[t]
\centering
\caption{\textbf{(a)} Construction of the honest student super-source
$S^{*}$ on the 401-image leak-isolated meta-validation set
(single-source; reproducible from archived predictions).
\textbf{(b)} Contribution on the official held-out test set---the only
leak-free instrument for the full pipeline, as V12 is leaked on
meta-validation.}
\label{tab:ablation}
\small
\begin{tabular}{lcc}
...
Table~\ref{tab:ablation}(a) decomposes the honest student source:
pseudo-distillation is the largest single lever ($+0.040$), as the soft
signal in the teacher's confidence scores compensates for the small
training set; multi-resolution TTA adds $+0.036$ because thin stripes a
few pixels tall are resolved unevenly by YOLO's strided feature pyramid
at any single input size; and dropping the domain-overlapping fold-0 is
net-positive because correlated predictions drag the WBF consensus.
Table~\ref{tab:ablation}(b) is the honest counterpart, and the more
important one: on genuinely unseen data the entire stack adds only
$+0.0021$ over submitting the teacher and $+0.0056$ over a plain 5-fold
ensemble. The $+0.077$ student source is, on test, largely redundant
with the teacher, which already encodes the same multi-fold knowledge;
the meta-validation ceiling (0.515) is moreover unbeatable there
because V12 is leaked on that split. The stack also does not extend:
raising the blend weight to $w{=}0.375$ gave 0.4768, and a more
aggressive second-generation pseudo-FT blend regressed to 0.4566
(below the teacher). Intermediate distillation stages were scored only
on meta-validation, as official submissions were rate-limited to one
per 12\,h.
...
ClearSAR Track~1, on a single cloud NVIDIA A100 GPU. The operationally
honest takeaway, however, is that a quality-weighted multi-fold
ensemble teacher recovers most of the attainable accuracy on unseen
SAR data; the distillation--TTA--blend stack is a small, fragile top-up
worthwhile only at a conservative blend weight. The largest remaining
gap is at high IoU thresholds, where sub-pixel height accuracy
dominates and a 640-px input under-resolves the median
$\approx$10-px stripe; sliced \emph{training} at 320-px tiles is the
