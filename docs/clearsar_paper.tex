...
the domain-overlapping fold before final fusion. On a 401-image
leak-isolated meta-validation set, the distilled student source gains
$+0.077$ mAP$_{50\text{-}95}$ over a cold per-fold baseline
(pseudo-distillation $+0.040$, multi-resolution TTA $+0.035$, fold
dropping $+0.002$). On the held-out test set this source is redundant
with the ensemble teacher: the full distillation--TTA--blend stack adds
only $+0.0021$ over submitting the
teacher alone, and $+0.0056$ over a plain 5-fold ensemble. We report
both measurements; the gap between them is the paper's main finding.
\end{abstract}
...
\begin{itemize}[noitemsep,leftmargin=*]
  \item We adapt a YOLO26x detector to RFI stripe geometry with a
  pixel-space NWD regression loss and SAR-aware augmentation, recovering
  a usable gradient for thin, non-overlapping boxes
  (Section~\ref{sec:method}).
  \item We assemble an ensemble-teacher pseudo-label distillation,
  multi-resolution TTA and selective cross-fold fusion pipeline that
  reaches 0.4776 mAP$_{50\text{-}95}$ (28th of 172) on the held-out test
  set (Section~\ref{sec:method}).
  \item We evaluate each component on a leak-isolated meta-validation
  set and on the held-out test set, showing the distilled source gains
  $+0.077$ mAP$_{50\text{-}95}$ in isolation but only $+0.0021$ over the
  teacher on unseen data (Section~\ref{sec:exp}).
\end{itemize}

\section{RELATED WORK}
\label{sec:rw}

\textbf{SAR and small object detection.} Deep-learning SAR detection
has been studied extensively for ships \cite{li2017ssdd,wang2019sarship}
and aircraft \cite{chen2016sartarget}; these targets are compact
(aspect ratios $\approx$1--4:1), unlike RFI stripes. Standard detectors
also struggle below 32$\times$32\,px owing to limited feature-map
resolution and gradient imbalance \cite{liu2021smallsurvey}, motivating
feature pyramids \cite{lin2017fpn}, transformer decoders
\cite{carion2020detr}, and modified regression losses. Normalised
Wasserstein Distance (NWD) \cite{wang2021nwd} replaces IoU regression
with a Wasserstein distance between Gaussians centred on predicted and
target boxes, yielding non-zero gradients for non-overlapping small
targets.
...
student on a stronger teacher's predictions. When the teacher is a
multi-fold ensemble and the student is fine-tuned on unlabelled
in-distribution data, this is termed noisy-student training
\cite{xie2020noisystudent} or self-training \cite{zoph2020rethinking}.

\textbf{Test-time augmentation and WBF.} WBF \cite{solovyev2021wbf}
clusters overlapping detections and averages their coordinates,
avoiding the precision--recall trade-off of NMS-based fusion, and is
widely used when combining predictions across resolutions.
...
We use YOLO26x \cite{ultralytics2024yolo11} ($\approx$59\,M
...
detail to note is that ultralytics evaluates the box loss on
coordinates normalised to the network input size, not in pixels. We
rescale the positive boxes to pixel units before computing NWD so that
$C$ is interpreted in pixel space; computing NWD on the normalised
coordinates directly leaves $C$ far too large, saturates the
similarity to a near-constant, and removes the gradient in the
small-height regime characteristic of RFI stripes (median
$\approx$10\,px).

\textbf{Hyperparameters.} Base training runs for up to 60 epochs
(early stopping, patience 15) with the MuSGD optimiser (ultralytics
8.4.47), initial learning rate $\eta_0{=}10^{-3}$ decaying linearly to
$\eta_f{=}10^{-5}$ over 3 warmup epochs, momentum 0.937, weight decay
$5{\times}10^{-4}$, batch 32, 640\,px resolution, and box/cls/dfl loss
gains $12.0/0.5/1.5$. Mosaic ($p{=}1.0$) and light mixup ($p{=}0.1$)
are enabled, with mosaic disabled for the final 10 epochs for clean
...
786-image test partition: submissions are uploaded as a COCO-format
JSON of detections, and the platform returns the official score
(rate-limited to one submission per 12\,h).

\textbf{Role of cross-validation.} The five-fold split serves an
\emph{ensemble-construction} role, not the conventional role of an
out-of-fold generalisation estimator. Each fold detector becomes a
member of the WBF teacher ensemble (Section~\ref{sec:pseudo}); the
per-fold \emph{validation} mAPs are never used to estimate test
performance or to select the final model. Cross-fold validation mAPs on
the full training set are inflated (``leaked''), because every fold
detector has, through the other four folds, seen the images held out by
any single fold. We therefore measure generalisation with two separate
sets: (i)~the 401-image leak-isolated meta-validation set
(Table~\ref{tab:ablation}), used only as a single-model gate, and
(ii)~the official held-out test set (Table~\ref{tab:heldout}), used for
all model selection and final reporting.

\textbf{Leak isolation.} The 401-image meta-validation set is sampled
exclusively from the fold-0 validation pool (seed 42, stratified by box
count, small-object count and aspect ratio; all new models train on the
remaining 2{,}753 images). It is leak-free only for models whose
training partition does not overlap it: the cold fold-0 detector and
the pseudo-FT students (trained on the meta-training pool) are clean,
whereas the teacher ensemble V12, which includes folds 1--4 each
trained on all of fold~0, is leaked on meta-validation. Meta-validation
therefore cannot judge V12 or any blend that contains it
(Section~\ref{sec:heldout}); the held-out test set is the only
leak-free measurement for the full pipeline.

\subsection{What transfers to genuinely unseen data}
\label{sec:heldout}

We measure how much each stage contributes on data no part of the
pipeline has seen, using the official held-out test set
(Table~\ref{tab:heldout}). This separates two quantities the
meta-validation chain conflates: the strength of the student source in
\emph{isolation}, and its \emph{marginal} value once added to the
teacher.

\begin{table}[t]
\centering
\caption{Contribution on the official held-out test set (the only
leak-free instrument for the full pipeline). All three rows were scored
by the challenge platform on the undisclosed 786-image partition. V12
adds an RF-DETR member and a CLAHE-retrained fold to the plain
ensemble; V17 adds the distillation super-source (pseudo-FT $+$
multi-resolution TTA $+$ fold-drop $+$ blend).}
\label{tab:heldout}
\small
\begin{tabular}{lcc}
\toprule
...
On unseen data, the full distillation, multi-resolution TTA, selective
fold-dropping and blending stack adds only $+0.0021$
mAP$_{50\text{-}95}$ over submitting the teacher ensemble, and
$+0.0056$ over a plain quality-weighted 5-fold WBF. The pipeline does
not extend either: raising the blend weight to $w{=}0.375$ regressed to
0.4768 ($-0.0008$), and a second-generation pseudo-FT blend regressed
to 0.4566 ($-0.019$, below the teacher). The marginal value of the
super-source over the teacher is small, and positive only at a
conservative blend weight.

\subsection{Decomposing the student source (meta-validation)}

The large meta-validation gains measure the construction of the student
source in isolation, on the leak-isolated 401-image set
(Table~\ref{tab:ablation}); every row is clean (the students train on
the meta-training pool only). The chain is reproducible from the
archived per-fold/per-resolution predictions
(\texttt{scripts/verify\_ablation.py}).

\begin{table}[t]
\centering
\caption{Construction of the student super-source $S^{*}$ on the
401-image leak-isolated meta-validation set, with the size- and
IoU-stratified breakdown (pycocotools). AP is mAP$_{50\text{-}95}$;
AP$_S$ is the small-area average (COCO area $<32^2$\,px), the hardest
RFI regime. These are single-source numbers; they do \emph{not} include
the (leaked) teacher.}
\label{tab:ablation}
\small
...
\begin{tabular}{lcccc}
\toprule
\textbf{Configuration} & \textbf{AP} & \textbf{AP$_{50}$} & \textbf{AP$_{75}$} & \textbf{AP$_S$} \\
\midrule
...
+ 3-fold WBF (drop $f_0$) $=S^{*}$ & \textbf{0.471} & \textbf{0.752} & \textbf{0.522} & \textbf{0.418} \\
\bottomrule
\end{tabular}
\end{table}

The AP chain $0.394 \to 0.434 \to 0.469 \to 0.471$ totals $+0.077$, with
pseudo-distillation the single largest component (deltas $+0.040$,
$+0.035$, $+0.002$). Small-box accuracy improves in step (AP$_S$
$0.356\to0.418$), while AP$_{75}$ stays the weakest column ($0.522$ on
$S^{*}$ versus $0.752$ at AP$_{50}$), locating the residual error at
high IoU, where sub-pixel stripe-height accuracy dominates. The contrast with
Table~\ref{tab:heldout} is the main result: the $+0.077$ buys a source
that is, on unseen test, redundant with the teacher ensemble, which
already encodes the same multi-fold knowledge. Adding $S^{*}$ to V12
cannot improve meta-validation (V12 is leaked there, so its 0.515
ceiling is unreachable by a leak-free source) and improves held-out
test by only $+0.0021$. The intermediate distillation stages were not
measured on held-out test: each official submission was rate-limited to
one per 12\,h, so only the three
ensemble end-points in Table~\ref{tab:heldout} were scored on the true
test partition.
...
Table~\ref{tab:blend} pairs the leaked fold-0 validation
mAP$_{50\text{-}95}$ with the official held-out test score at three
blend weights. V12 is itself leaked on fold-0 val, so adding the
leak-free super-source decreases leaked val monotonically while
improving held-out test up to $w{=}0.25$.
...
The cold per-fold baseline saturates around 0.394
mAP$_{50\text{-}95}$ (Table~\ref{tab:ablation}, top row): 2{,}753
images are too few to support reliable localisation of thin,
low-contrast RFI stripes across the full IoU=0.50--0.95 sweep. Building
the student source recovers $+0.077$ on meta-validation in three steps.
\emph{(i)}
Pseudo-distillation from a multi-source ensemble teacher is the largest
single component ($+0.040$): the soft signal in the teacher's
...
\emph{(ii)}
Multi-resolution TTA between 608 and 704\,px adds $+0.035$
...
Dropping the domain-overlapping fold-0 from the cross-fold ensemble, a
\emph{smaller} ensemble, is net-positive because correlated predictions
drag the WBF consensus when one source has seen the validation data
during training.

This gain does not transfer. Almost all of the $+0.077$ is captured
independently by the teacher ensemble: on the held-out test set
(Table~\ref{tab:heldout}), adding the student super-source to the
teacher gives only $+0.0021$, and a higher blend weight or a second
pseudo-FT generation regresses below the teacher. The practical
recommendation is therefore that a quality-weighted multi-fold ensemble
recovers most of the attainable accuracy on a fresh SAR acquisition,
and the distillation--TTA--blend stack adds little beyond it for its
extra compute.

Several directions that improved leaked cross-fold validation did
...
The submission placed the \emph{UdeM AI} team 28th of 172 teams on
