...
leak-isolated meta-validation set, the honest student source built by
this pipeline gains $+0.077$ mAP$_{50\text{-}95}$ over a cold per-fold
baseline (pseudo-distillation $+0.040$, multi-resolution TTA $+0.036$,
fold dropping $+0.002$). On the genuinely unseen test set, however,
this source is largely redundant with the ensemble teacher: the full
distillation--TTA--blend stack adds only $+0.0021$ over submitting the
teacher alone, and $+0.0056$ over a plain 5-fold ensemble. We report
both measurements explicitly, as the gap between them is the paper's
central, and cautionary, finding.
\end{abstract}
...
\textbf{Role of cross-validation.} We stress that the five-fold split
serves an \emph{ensemble-construction} role, not the conventional role
of an out-of-fold generalisation estimator. Each fold detector becomes
a member of the WBF teacher ensemble (Section~\ref{sec:pseudo}); the
folds exist to give the ensemble diverse members, and the per-fold
\emph{validation} mAPs are never used to estimate test performance or
to select the final model. This is deliberate: cross-fold validation
mAPs on the full training set are inflated (``leaked''), because every
fold detector has, through the other four folds, seen the images held
out by any single fold. Honest generalisation is therefore measured by
two separate instruments: (i)~the 401-image leak-isolated
meta-validation set (Table~\ref{tab:ablation}), used only as a
single-model gate, and (ii)~the official held-out test set
(Table~\ref{tab:heldout}), used for all model-selection and final
reporting.
...
remaining 2{,}753 images). It is honest only for models whose training
partition does not overlap it: the cold fold-0 detector and the
pseudo-FT students (trained on the meta-training pool) are clean,
whereas the full teacher ensemble V12---which includes folds 1--4, each
trained on all of fold~0---remains \emph{leaked} on meta-validation.
This is why meta-validation cannot be used to judge V12 or any blend
that contains it (Section~\ref{sec:heldout}), and why the held-out test
set is the only honest instrument for the full pipeline.

\subsection{What transfers to genuinely unseen data}
\label{sec:heldout}

The decisive question---raised directly in review---is how much each
stage contributes on data no part of the pipeline has seen, rather than
on meta-validation. We answer it on the official held-out test set
(Table~\ref{tab:heldout}), separating two things the meta-validation
chain conflates: the strength of the honest student source in
\emph{isolation}, and its \emph{marginal} value once added to an
already-strong teacher.
...
The honest reading is sobering and we state it plainly: on unseen data,
the entire distillation, multi-resolution TTA, selective fold-dropping
and blending stack adds only $+0.0021$ mAP$_{50\text{-}95}$ over simply
submitting the teacher ensemble, and $+0.0056$ over a plain
quality-weighted 5-fold WBF. The pipeline does not \emph{extend}
either: raising the blend weight to $w{=}0.375$ regressed to 0.4768
($-0.0008$), and a more aggressive second-generation pseudo-FT blend
regressed to 0.4566 ($-0.019$, below the teacher). The marginal value
of the learned super-source over the teacher is real but small, and
positive only at a conservative blend weight.

\subsection{Decomposing the student source (meta-validation)}

Where, then, do the large meta-validation gains come from? They measure
the construction of the honest student source in isolation, on the
leak-isolated 401-image set (Table~\ref{tab:ablation}); every row here
is clean (the students train on the meta-training pool only). The chain
is reproducible from the archived per-fold/per-resolution predictions
(\texttt{scripts/verify\_ablation.py}).

\begin{table}[t]
\centering
\caption{Construction of the honest student super-source $S^{*}$, on the
401-image leak-isolated meta-validation set. These are single-source
numbers; they do \emph{not} include the (leaked) teacher.}
\label{tab:ablation}
\small
\begin{tabular}{lcc}
\toprule
...
+ Multi-res TTA (4 res, WBF)          & 0.469 & $+0.036$ \\
+ 3-fold WBF (drop $f_0$) $=S^{*}$    & 0.471 & $+0.002$ \\
\bottomrule
\end{tabular}
\end{table}

The chain $0.394 \to 0.434 \to 0.469 \to 0.471$ totals $+0.077$, and
pseudo-distillation is the single largest honest lever. The apparent
contradiction with Table~\ref{tab:heldout} is the central lesson: the
$+0.077$ buys an honest source that is, on unseen test, \emph{largely
redundant} with the teacher ensemble, which already encodes the same
multi-fold knowledge. Adding $S^{*}$ to V12 cannot help on
meta-validation at all---V12 is leaked there, so its 0.515 ceiling is
unbeatable by an honest source---and helps only marginally
($+0.0021$) on held-out test. We could not measure the intermediate
distillation stages directly on held-out test: each official
submission was rate-limited to one per 12\,h, so only the three
...
the honest student source then recovers $+0.077$ on meta-validation,
with three contributing steps. \emph{(i)}
Pseudo-distillation from a multi-source ensemble teacher is the largest
single honest lever ($+0.040$): the soft signal in the teacher's
...
\emph{(ii)}
Multi-resolution TTA between 608 and 704\,px adds a further $+0.036$
...
--- counter-intuitively, a \emph{smaller} ensemble --- is
...
The harder, and more honest, conclusion concerns
\emph{transfer}. Almost all of that $+0.077$ is captured independently
by the teacher ensemble: on the genuinely unseen test set
(Table~\ref{tab:heldout}), adding the entire student super-source to
the teacher buys only $+0.0021$, and pushing the same machinery harder
(higher blend weight, a second pseudo-FT generation) regresses below
the teacher. For a practitioner facing a fresh, unlabelled SAR
acquisition, the operationally honest recommendation is therefore that
a quality-weighted multi-fold ensemble teacher recovers most of the
attainable accuracy; the distillation--TTA--blend stack is a small,
fragile top-up, worthwhile only at a conservative blend weight and at
substantial extra compute. We report this gap rather than the
flattering meta-validation chain alone, because it is what generalises.
