...
\textbf{Hyperparameters.} Base training runs for up to 100 epochs
(early stopping, patience 15) with the MuSGD optimiser, initial
learning rate $\eta_0{=}10^{-3}$ decaying linearly to
$\eta_f{=}10^{-5}$, 3 warmup epochs, batch 32, 640\,px resolution,
box-loss weight 12.0.
...
by the challenge platform on the undisclosed 786-image partition.}
\label{tab:heldout}
\small
\begin{tabular}{lcc}
\toprule
...
Full pipeline V17 (\,$+$distill\,$+$TTA\,$+$drop-$f_0$\,$+$blend) & \textbf{0.4776} & $+0.0021$ \\
\bottomrule
\end{tabular}
\end{table}
...
ceiling; adding the leak-free super-source decreases leaked val
